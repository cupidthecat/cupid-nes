...
`--apu-disable-noise-mode` keeps `$400E`'s stored mode flag but forces the bit-one feedback tap at each LFSR clock. The default still selects bit one or bit six from that flag. Regional period tables, envelopes, and length counters use their normal paths. `--apu-swap-duty-cycles` exchanges duty selections 1 and 2 when software writes `$4000` or `$4004`; fields 0 and 3 are unchanged. It applies only to the two base pulse channels. MMC5 pulse registers and expansion-audio duty sequences retain their ordinary behavior. These controls are independent of the DMC CPU revision and survive reset.
...
`--ppu-disable-oamdata-read` leaves `$2004` reads on the PPU I/O latch, including normal open-bus decay, without performing the OAM read or refreshing the latch. `--ppu-disable-palette-readback` returns the existing read-buffer byte for palette-space `$2007` reads and schedules the ordinary external transfer. The default palette-readback path retains its grayscale mask and upper open-bus bits. Both settings keep register-recovery timing and the ordinary CPU/PPU clock path.
...
`--ppu-sprite-eval-wrap-bug` models the early revision's behavior after primary-OAM evaluation wraps. When secondary OAM is not full, evaluation can copy a Y byte into the next slot while leaving its tile, attributes, and X bytes at `$FF`. The normal sprite fetch and shifter path then determines whether that entry produces a pixel at X=255. The regression covers zero through seven complete sprites, both sprite heights, wrapped Y coordinates inside and outside the scanline, and the resulting pattern address, flips, palette, and pixel. The control does not itself set sprite-zero or overflow flags. It is independent of `--ppu-revision` and the other optional PPU controls, defaults to off, and survives reset.
...
CPU reset clears earlier NMI requests before its bus cycles. A new edge during those cycles remains latched for the normal interrupt poll after an instruction. With `--ppu-reset-suppression`, the PPU can reach vblank and raise that edge during reset. The [reset regressions](../src/tests/cpu_accuracy_reset_nmi.h) advance the real PPU across this window at five offsets in each region, verify delivery after the first instruction at the reset vector, and check that a held line does not trigger a second NMI. Paired cases cover ordinary PPU reset, which clears the NMI-enable control.
...
[Native mixed-CHR tests](../src/tests/native_mixed_chr_accuracy.c) load 4 KiB of volatile CHR RAM followed by 4 KiB of NVRAM through the production loader, access both portions through CPU instructions and PPUDATA, and compare the complete save file. CPROM covers unequal allocations and bank wrapping. UNROM 512 covers CHR-backed nametables, their pattern-bank alias, and save offsets relative to the NVRAM tail. Reloads must restore persistent bytes and reinitialize volatile bytes. Additional cases retain short-save initialization, RAM-only and NVRAM-only images, protected open bus, ROM-adjacent storage, and the active cartridge after a rejected replacement.
