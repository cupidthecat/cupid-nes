...
void nes_capture_options_defaults(NesCaptureOptions *options) {
    if (options) *options = (NesCaptureOptions){44100, UINT32_MAX, true};
}
...
static void release_implementation(NesCaptureSession *session, bool abort_stream) {
    ...
    if (abort_stream) nes_capture_abort(&implementation->stream);
...
static void observe_audio(void *context, unsigned machine, double sample_rate,
                            float left, float right) {
    (void)sample_rate;
    NesCaptureSession *session = context;
    CaptureImplementation *implementation = session->implementation;
    if (!implementation || !implementation->frame_open || machine >= implementation->machines
        || implementation->frame_error != NES_FILE_OK) return;
...
NesFileResult nes_capture_session_start(NesCaptureSession *session, const char *path,
    ...
    if (!session || !options) return NES_FILE_INVALID_ARGUMENT;
    session->error[0] = '\0';
    ...
    if (options->sample_rate < 8000 || options->sample_rate > 192000
        || options->byte_limit > UINT32_MAX
        || (video && (!frame || !frame->pixels))) return NES_FILE_INVALID_ARGUMENT;
    CaptureImplementation *implementation = calloc(1, sizeof(*implementation));
    if (!implementation) return NES_FILE_OUT_OF_MEMORY;
    ...
    implementation->pcm_capacity = (size_t)(((uint64_t)options->sample_rate * rate.denominator
                                             + rate.numerator - 1u) / rate.numerator);
    ...
    NesFileResult result = video
        ? nes_capture_avi_open(path, frame->width, frame->height, options->sample_rate,
                                rate.numerator, rate.denominator, options->byte_limit, &implementation->stream)
        : nes_capture_wav_open(path, options->sample_rate, options->byte_limit, &implementation->stream);
    ...
    session->info = (NesCaptureInfo){
        .recording = true, .video = video, .width = video ? frame->width : 0,
        .height = video ? frame->height : 0, .sample_rate = options->sample_rate,
        .frame_rate = rate, .bytes = nes_capture_file_size(implementation->stream)
    };
    return NES_FILE_OK;
}

NesFileResult nes_capture_session_stop(NesCaptureSession *session) {
    if (!session) return NES_FILE_INVALID_ARGUMENT;
    CaptureImplementation *implementation = session->implementation;
    if (!implementation) return NES_FILE_OK;
    session->info.bytes = nes_capture_file_size(implementation->stream);
    NesFileResult result = nes_capture_close(&implementation->stream);
    if (result != NES_FILE_OK)
...
static NesFileResult stop_with_error(NesCaptureSession *session, NesFileResult result,
                                       const char *message) {
    NesFileResult finalization = nes_capture_session_stop(session);
    if (finalization != NES_FILE_OK) return finalization;
...
bool nes_capture_session_begin_frame(NesCaptureSession *session) {
    if (!session || !session->implementation) return false;
    CaptureImplementation *implementation = session->implementation;
    if (implementation->image_crc != rom_file_crc32() || implementation->region != nes_timing()->region
        ...
        (void)stop_with_error(session, NES_FILE_INVALID_ARGUMENT, "Recording stopped when the emulation session changed");
        return false;
    }
    if (implementation->frame_open) return true;
    for (unsigned i = 0; i < implementation->machines; ++i) {
        ...
        if (channel->have_previous && channel->previous.cycle > channel->start_cycle) {
            (void)stop_with_error(session, NES_FILE_INVALID_ARGUMENT, "Recording stopped after the machine clock was restored");
            return false;
        }
...
NesFileResult nes_capture_session_end_frame(NesCaptureSession *session,
    ...
    if (!session || !session->implementation) return NES_FILE_OK;
    CaptureImplementation *implementation = session->implementation;
    if (!implementation->frame_open) return NES_FILE_OK;
    if (implementation->frame_error != NES_FILE_OK)
        return stop_with_error(session, implementation->frame_error, "Recording stopped because its input buffer or clock changed");
    if (implementation->image_crc != rom_file_crc32() || implementation->region != nes_timing()->region)
        return stop_with_error(session, NES_FILE_INVALID_ARGUMENT, "Recording stopped when the emulation session changed");
    uint64_t cycles[2] = {0};
    for (unsigned i = 0; i < implementation->machines; ++i) {
        uint64_t end = vs_side_cpu_cycles(i);
        if (end <= implementation->channels[i].start_cycle)
            return stop_with_error(session, NES_FILE_INVALID_ARGUMENT, "Recording stopped because a completed frame had no forward emulation time");
    ...
    NesFileResult result = session->info.video
    ...
    if (result != NES_FILE_OK) {
        const char *message = result == NES_FILE_TOO_LARGE
            ? "Recording reached its configured file or index limit"
            : result == NES_FILE_INVALID_ARGUMENT ? "Recording stopped when the output dimensions changed"
    ...
    for (unsigned i = 0; i < implementation->machines; ++i) {
        CaptureChannel *channel = &implementation->channels[i];
        if (channel->count) channel->previous = channel->samples[channel->count - 1u];
    }
