...
typedef enum { NES_CAPTURE_CODEC_RAW, NES_CAPTURE_CODEC_ZMBV } NesCaptureCodec;
...
NesFileResult nes_capture_png_memory(const NesCaptureFrame *frame, uint8_t **data, size_t *size);
...
NesFileResult nes_capture_wav_open(const char *path, unsigned sample_rate, uint64_t byte_limit, NesCaptureStream **out);
