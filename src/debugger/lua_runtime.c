...
typedef enum { LUA_CALLBACK_READ, LUA_CALLBACK_WRITE, LUA_CALLBACK_EXEC } LuaCallbackType;
...
static int lua_read_word(lua_State *state) {
    lua_Integer address = luaL_checkinteger(state, 1);
    luaL_argcheck(state, address >= 0 && address < 0xFFFF, 1, "word address out of range");
    uint8_t lo = debugger_peek_cpu((uint16_t)address);
    uint8_t hi = debugger_peek_cpu((uint16_t)(address + 1));
    lua_pushinteger(state, (lua_Integer)(lo | ((uint16_t)hi << 8)));
    return 1;
}

static int lua_read_range(lua_State *state) {
    lua_Integer address = luaL_checkinteger(state, 1);
    lua_Integer count = luaL_checkinteger(state, 2);
    luaL_argcheck(state, address >= 0 && address <= 0xFFFF, 1, "address out of range");
    luaL_argcheck(state, count >= 0 && count <= 4096 && count <= 0x10000 - address, 2,
                  "range must fit the CPU address space and contain at most 4096 bytes");
    lua_createtable(state, (int)count, 0);
    for (lua_Integer i = 0; i < count; ++i) {
        lua_pushinteger(state, debugger_peek_cpu((uint16_t)(address + i)));
...
static int lua_frame_count(lua_State *state) {
    lua_pushinteger(state, (lua_Integer)ppu.frame_count);
    return 1;
}

static int lua_region(lua_State *state) {
    lua_pushstring(state, nes_region_name(nes_timing()->region));
    return 1;
}
...
static int lua_get_register(lua_State *state) {
    ...
    if (!strcmp(name, "A") || !strcmp(name, "a")) {
        ...
        value = cpu.x;
    } else if (!strcmp(name, "Y") || !strcmp(name, "y")) {
        value = cpu.y;
    } else if (!strcmp(name, "PC") || !strcmp(name, "pc")) {
        value = cpu.pc;
    } else if (!strcmp(name, "SP") || !strcmp(name, "sp")) {
        value = cpu.sp;
    } else if (!strcmp(name, "P") || !strcmp(name, "p") || !strcmp(name, "status")) {
        value = cpu.status;
    } else {
        return luaL_error(state, "unknown CPU register");
    }
...
static int lua_set_register(lua_State *state) {
    ...
    if (!debugger_set_cpu_register(name, (uint32_t)value)) {
        return luaL_error(state, "unknown CPU register");
    }
    return 0;
}
...
static bool parse_callback_type(const char *name, LuaCallbackType *type) {
    if (!strcmp(name, "read")) {
        *type = LUA_CALLBACK_READ;
    } else if (!strcmp(name, "write")) {
        *type = LUA_CALLBACK_WRITE;
    } else if (!strcmp(name, "exec") || !strcmp(name, "execute")) {
        *type = LUA_CALLBACK_EXEC;
    } else {
        return false;
    }
    return true;
...
static int lua_add_callback(lua_State *state) {
    ...
    luaL_argcheck(state, last >= first && last <= 0xFFFF, top >= 4 ? 3 : 2, "invalid callback range");
    luaL_checktype(state, function_index, LUA_TFUNCTION);
    LuaCallbackType type;
    if (!parse_callback_type(name, &type)) {
        return luaL_error(state, "callback type must be read, write, or exec");
    }
    if (runtime.callback_count >= LUA_CALLBACK_LIMIT) {
        return luaL_error(state, "too many Lua callbacks");
    }
...
static int lua_remove_callback(lua_State *state) {
    ...
    for (size_t i = 0; i < runtime.callback_count; ++i) {
        if (runtime.callbacks[i].id != (uint32_t)id) {
            continue;
        }
...
static void put_pixel(int x, int y, uint32_t color) {
    if (x < 0 || x >= DEBUG_LUA_OVERLAY_WIDTH || y < 0 || y >= DEBUG_LUA_OVERLAY_HEIGHT) {
        return;
    }
...
static int lua_coordinate(lua_State *state, int index, int minimum, int maximum) {
    ...
    luaL_argcheck(state, value >= minimum && value <= maximum, index, "coordinate is too far outside the overlay");
    return (int)value;
}
...
static int lua_draw_line(lua_State *state) {
    ...
    if ((x0 < 0 && x1 < 0) || (x0 >= DEBUG_LUA_OVERLAY_WIDTH && x1 >= DEBUG_LUA_OVERLAY_WIDTH) || (y0 < 0 && y1 < 0) ||
        (y0 >= DEBUG_LUA_OVERLAY_HEIGHT && y1 >= DEBUG_LUA_OVERLAY_HEIGHT)) {
        return 0;
    }
    if (x0 < 0) {
        x0 = 0;
    } else if (x0 >= DEBUG_LUA_OVERLAY_WIDTH) {
        x0 = DEBUG_LUA_OVERLAY_WIDTH - 1;
    }
    if (x1 < 0) {
        x1 = 0;
    } else if (x1 >= DEBUG_LUA_OVERLAY_WIDTH) {
        x1 = DEBUG_LUA_OVERLAY_WIDTH - 1;
    }
    if (y0 < 0) {
        y0 = 0;
    } else if (y0 >= DEBUG_LUA_OVERLAY_HEIGHT) {
        y0 = DEBUG_LUA_OVERLAY_HEIGHT - 1;
    }
    if (y1 < 0) {
        y1 = 0;
    } else if (y1 >= DEBUG_LUA_OVERLAY_HEIGHT) {
        y1 = DEBUG_LUA_OVERLAY_HEIGHT - 1;
    }
...
static int lua_draw_rect(lua_State *state) {
    ...
    luaL_argcheck(state, requested_width >= 0 && requested_width <= DEBUG_LUA_OVERLAY_WIDTH * 3, 3,
                  "rectangle width is out of range");
    luaL_argcheck(state, requested_height >= 0 && requested_height <= DEBUG_LUA_OVERLAY_HEIGHT * 3, 4,
                  "rectangle height is out of range");
...
#define SET_INT(name, value)                                                                                           \
    ...
    SET_INT("ctrl", snapshot.ctrl);
    SET_INT("mask", snapshot.mask);
    SET_INT("status", snapshot.status);
    SET_INT("v", snapshot.v);
    SET_INT("t", snapshot.t);
    SET_INT("fineX", snapshot.fine_x);
    SET_INT("scanline", snapshot.scanline);
    SET_INT("dot", snapshot.dot);
    SET_INT("frame", snapshot.frame);
    SET_INT("cycles", snapshot.ppu_cycles);
#undef SET_INT
    return 1;
}
...
static void install_emu_api(lua_State *state) {
    static const luaL_Reg functions[] = {{"read", lua_read_cpu},
                                         {"readWord", lua_read_word},
                                         {"readRange", lua_read_range},
                                         {"readPpu", lua_read_ppu},
                                         {"write", lua_write_cpu},
                                         {"getRegister", lua_get_register},
                                         {"setRegister", lua_set_register},
                                         {"getPpuState", lua_get_ppu_state},
                                         {"getCpuCycleCount", lua_get_cpu_cycles},
                                         {"getFrameCount", lua_frame_count},
                                         {"getRegion", lua_region},
                                         {"addMemoryCallback", lua_add_callback},
                                         {"removeMemoryCallback", lua_remove_callback},
                                         {"pause", lua_pause},
                                         {"resume", lua_resume_execution},
                                         {"log", lua_log_message},
                                         {"drawPixel", lua_draw_pixel},
                                         {"drawLine", lua_draw_line},
                                         {"drawRectangle", lua_draw_rect},
                                         {"clearOverlay", lua_clear_overlay},
                                         {NULL, NULL}};
...
bool debugger_lua_load(const char *name, const char *source) {
    ...
    if (!nes_execution_allows_host_configuration()) {
        return false;
    }
    debugger_lua_unload();
...
uint32_t debugger_lua_instruction_budget(void) {
    return runtime.instruction_budget;
}
...
void debugger_lua_clear_overlay(void) {
    memset(runtime.overlay, 0, sizeof(runtime.overlay));
}
...
static bool call_callback(LuaMemoryCallback *callback, uint16_t address, uint8_t *value, bool can_replace) {
...
static void dispatch_callbacks(LuaCallbackType type, uint16_t address, uint8_t *value, bool can_replace) {
    if (!runtime.loaded || runtime.faulted || runtime.inside_callback) {
        return;
    }
    for (size_t i = 0; i < runtime.callback_count; ++i) {
        LuaMemoryCallback *callback = &runtime.callbacks[i];
        if (!callback_matches(callback, type, address)) {
            continue;
        }
        if (!call_callback(callback, address, value, can_replace)) {
            break;
        }
    }
}
