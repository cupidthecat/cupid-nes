...
void HQX_CALLCONV hqxInit(void)
{
    ...
    for (c = 0; c < 16777216; c++) {
        ...
        /* Chroma can be negative before the offset. Convert through a signed
         * integer to preserve truncation without an out-of-range unsigned cast. */
        u = (uint32_t)((int32_t)(-0.169*r - 0.331*g + 0.5*b) + 128);
        v = (uint32_t)((int32_t)(0.5*r - 0.419*g - 0.081*b) + 128);
