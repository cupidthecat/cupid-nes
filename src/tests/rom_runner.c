...
static int load_test_image(const char *path) {
...
static int run_image(const char *path, unsigned limit, const char *frame_path, bool enable_mmc3_ram) {
    if (load_test_image(path)) return 1;
    if (enable_mmc3_ram) {
    ...
    while (frames < limit) {
        ...
        if (!frame_path && ((++instructions & 1023) == 0)) {
            ...
            if (protocol) {
                ...
                if (status == 0x81) {
                    printf("ROM %s: RESET REQUEST (%u frames)\n", path, frames);
                    print_test_output();
                    return 2;
                }
            }
...
int run_diagnostic_rom(const char *path, unsigned frame_limit) {
    int result = run_image(path, frame_limit, NULL, false);
    unload_rom();
    return result;
}

int render_diagnostic_rom(const char *path, unsigned frames, const char *output) {
    int result = run_image(path, frames, output, false);
    unload_rom();
    return result;
}

int run_mmc3_diagnostic_rom(const char *path, unsigned frame_limit) {
    int result = run_image(path, frame_limit, NULL, true);
    unload_rom();
    return result;
}
