...
static NesTasResult ctas_from_file(NesFileResult result) {
    ...
    case NES_FILE_OK:
        return NES_TAS_OK;
    case NES_FILE_INVALID_ARGUMENT:
        return NES_TAS_INVALID_ARGUMENT;
    case NES_FILE_OUT_OF_MEMORY:
        return NES_TAS_OUT_OF_MEMORY;
...
static void ctas_write_bytes(CtasWriter *writer, const void *data, size_t size) {
    if (!ctas_writer_reserve(writer, size)) {
        return;
    }
    if (size) {
...
static char *ctas_read_string(CtasReader *reader, size_t limit) {
    ...
    if (!ctas_reader_charge(reader, (size_t)length + 1)) {
        return NULL;
    }
...
static void ctas_read_frame(CtasReader *reader, NesFm2Frame *frame) {
    ...
    if (pads) {
        memcpy(frame->pads, pads, sizeof(frame->pads));
    }
    for (unsigned port = 0; port < 2; ++port) {
...
static void ctas_write_movie(CtasWriter *writer, const NesFm2Movie *movie) {
    ...
    if (!ctas_writer_reserve(writer, measured)) {
        return;
    }
    size_t written = 0;
    fm2 = nes_fm2_export(movie, NES_FM2_BINARY, writer->data + writer->size, writer->capacity - writer->size, &written,
                         &diagnostic);
...
static void ctas_read_movie(CtasReader *reader, NesFm2Movie *movie) {
    ...
    if (!ctas_reader_charge_array(reader, movie->frame_count, sizeof(NesFm2Frame))) {
        return;
    }
...
static void ctas_read_timeline(CtasReader *reader, TasTimeline *timeline) {
    ...
    if (frames) {
        ...
        if (!ctas_reader_charge(reader, frames)) {
            return;
        }
    ...
    if (marker_count) {
        ...
        if (!ctas_reader_charge_array(reader, (size_t)marker_count, sizeof(*timeline->markers))) {
            return;
        }
...
static void ctas_write_clipboard(CtasWriter *writer, const TasClipboard *clipboard) {
    ...
    for (size_t i = 0; i < clipboard->span; ++i) {
        ...
        if (clipboard->present[i]) {
            ctas_write_frame(writer, &clipboard->frames[i]);
        }
    }
}
...
static void ctas_read_clipboard(CtasReader *reader, TasClipboard *clipboard) {
    ...
    if (span64 > SIZE_MAX || count64 > span64 || span64 > SIZE_MAX / sizeof(*clipboard->frames) ||
        span64 > limits.max_frames || span64 > remaining || count64 > (remaining - (size_t)span64) / CTAS_FRAME_BYTES) {
        reader->result = NES_TAS_FORMAT_ERROR;
        return;
    }
    ...
    if (!ctas_reader_charge_array(reader, clipboard->span, sizeof(*clipboard->frames) + 1)) {
        return;
    }
...
static void ctas_read_history(CtasReader *reader, TasHistoryEntry **head, size_t *count, size_t *history_bytes,
    ...
    for (size_t i = 0; i < (size_t)count64; ++i) {
        if (!ctas_reader_charge(reader, sizeof(TasHistoryEntry))) {
            return;
        }
        ...
        if (first != UINT64_MAX && first > SIZE_MAX) {
            reader->result = NES_TAS_FORMAT_ERROR;
        }
        ...
        if (entry->first_changed != SIZE_MAX && entry->first_changed > entry->state.timeline.movie.frame_count) {
            tas_state_free(&entry->state);
            free(entry);
            reader->result = NES_TAS_FORMAT_ERROR;
            return;
...
static void ctas_read_source_modules(CtasReader *reader, NesTasProject *project) {
    ...
    for (unsigned i = 0; i < NES_FM3_PROJECT_MODULE_COUNT; ++i) {
        ...
        if (size) {
            if (!ctas_reader_charge(reader, size)) {
                return;
            }
...
/* Version 3 appends navigation metadata for the current state and each retained
 * undo/redo snapshot. The preceding state encoding remains compatible with v2. */
static void ctas_write_navigation(CtasWriter *writer, const TasProjectState *state) {
    ctas_write_u64(writer, state->navigation_count);
    for (size_t i = 0; i < state->navigation_count; ++i) {
        const TasNavigationBookmark *entry = &state->navigation[i];
        ctas_write_u64(writer, entry->frame);
        ctas_write_string(writer, entry->name, NES_TAS_BOOKMARK_NAME_MAX);
        ctas_write_string(writer, entry->note, NES_TAS_NAVIGATION_NOTE_MAX);
    }
}

static void ctas_read_navigation_text(CtasReader *reader, char *out, size_t limit, bool required) {
    uint32_t length = ctas_read_u32(reader);
    if (reader->result != NES_TAS_OK) {
        return;
    }
    if (length > limit || (required && !length)) {
        reader->result = NES_TAS_FORMAT_ERROR;
        return;
    }
    const uint8_t *text = ctas_read_bytes(reader, length);
    if (reader->result != NES_TAS_OK) {
        return;
    }
...
static void ctas_read_navigation(CtasReader *reader, TasProjectState *state) {
    uint64_t count = ctas_read_u64(reader);
    if (reader->result != NES_TAS_OK) {
        return;
    }
    if (count > NES_TAS_NAVIGATION_LIMIT || count > (reader->size - reader->offset) / 17u) {
        reader->result = NES_TAS_FORMAT_ERROR;
        return;
    }
    if (!count || !ctas_reader_charge_array(reader, (size_t)count, sizeof(*state->navigation))) {
        return;
    }
    state->navigation = calloc((size_t)count, sizeof(*state->navigation));
    if (!state->navigation) {
        reader->result = NES_TAS_OUT_OF_MEMORY;
        return;
    }
    state->navigation_count = state->navigation_capacity = (size_t)count;
    size_t previous = 0;
    for (size_t i = 0; i < (size_t)count && reader->result == NES_TAS_OK; ++i) {
        TasNavigationBookmark *entry = &state->navigation[i];
        uint64_t frame = ctas_read_u64(reader);
        if (frame > state->timeline.movie.frame_count || frame < previous) {
            reader->result = NES_TAS_FORMAT_ERROR;
            return;
        }
        entry->frame = previous = (size_t)frame;
        ctas_read_navigation_text(reader, entry->name, NES_TAS_BOOKMARK_NAME_MAX, true);
        ctas_read_navigation_text(reader, entry->note, NES_TAS_NAVIGATION_NOTE_MAX, false);
    }
}

static void ctas_read_navigation_history(CtasReader *reader, TasHistoryEntry *head, size_t *history_bytes) {
    for (TasHistoryEntry *entry = head; entry && reader->result == NES_TAS_OK; entry = entry->next) {
        ctas_read_navigation(reader, &entry->state);
        size_t extra = entry->state.navigation_capacity * sizeof(*entry->state.navigation);
        if (extra > SIZE_MAX - entry->bytes || extra > SIZE_MAX - *history_bytes) {
            reader->result = NES_TAS_FORMAT_ERROR;
            return;
        }
        entry->bytes += extra;
...
NesTasResult nes_tas_project_save(const NesTasProject *project, const char *path) {
    if (!project || !path || project->edit_active) {
        return NES_TAS_INVALID_ARGUMENT;
    }
    CtasWriter writer = {0};
    ...
    ctas_write_navigation(&writer, &project->state);
    for (const TasHistoryEntry *entry = project->undo; entry; entry = entry->next) {
        ctas_write_navigation(&writer, &entry->state);
    }
    for (const TasHistoryEntry *entry = project->redo; entry; entry = entry->next) {
        ctas_write_navigation(&writer, &entry->state);
    }
    if (writer.result == NES_TAS_OK) {
    ...
    NesTasResult result = ctas_from_file(nes_movie_save_atomic(path, writer.data, writer.size));
    free(writer.data);
    return result;
}
...
NesTasResult nes_tas_project_load(const char *path, NesTasProject **out) {
    ...
    if (file != NES_FILE_OK) {
        return ctas_from_file(file);
    }
    if (size < NES_CTAS_MAGIC_SIZE + 4 + 4) {
        free(data);
        return NES_TAS_FORMAT_ERROR;
    }
    uint32_t stored_crc = (uint32_t)data[size - 4] | (uint32_t)data[size - 3] << 8 | (uint32_t)data[size - 2] << 16 |
    ...
    if (project) {
        ...
        if (max_entries > SIZE_MAX || max_bytes > SIZE_MAX) {
            reader.result = NES_TAS_FORMAT_ERROR;
        }
        ...
        if (reader.result == NES_TAS_OK && version >= 3) {
            ctas_read_navigation(&reader, &project->state);
            ctas_read_navigation_history(&reader, project->undo, &project->history_bytes);
            ctas_read_navigation_history(&reader, project->redo, &project->history_bytes);
        }
        ...
        if (reader.result == NES_TAS_OK &&
            (project->undo_count > project->max_history_entries || project->redo_count > project->max_history_entries ||
    ...
    tas_history_describe_all(project);
    *out = project;
    return NES_TAS_OK;
}
