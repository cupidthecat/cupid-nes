...
static const Clay_Color ink = {232, 237, 247, 255}, muted = {151, 165, 190, 255},
                        accent = {170, 180, 255, 255};
static const char *const menus[] = {"File", "Emulation", "View", "Audio", "Media", "Tools", "Help"};
static const char *const categories[] = {"General",         "Emulation",        "Video",
                                         "Audio",           "Controllers",      "Media / firmware",
                                         "Files / storage", "Advanced hardware"};
static void label(FrontendDesktopUi *ui, const char *text, int size, Clay_Color color) {
    CLAY_TEXT(desktop_clay_string(ui->clay, text),
              CLAY_TEXT_CONFIG({.fontSize = (uint16_t)size, .textColor = color}));
}
...
static void button(FrontendDesktopUi *ui, const char *text, int kind, int index, int direction, bool active,
    ...
    CLAY(id, {.layout = {.padding = {12, 12, 8, 8}, .childAlignment = {.y = CLAY_ALIGN_Y_CENTER}},
        ...
        label(ui, text, 14, enabled ? active ? accent : ink : muted);
    }
}
...
static void settings(FrontendDesktopUi *ui, float height) {
    ...
    ui->visible_rows = (int)((height - 240) / 40);
    if (ui->visible_rows < 3)
        ui->visible_rows = 3;
    CLAY_AUTO_ID(
        {.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_GROW()}, .childGap = 16}}) {
        ...
        CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_GROW()},
            ...
            for (int i = start; i < count && i < start + ui->visible_rows; ++i) {
                ...
                CLAY_AUTO_ID(
                    {.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(34)},
                                .childGap = 5,
                                .childAlignment = {.y = CLAY_ALIGN_Y_CENTER},
                                .padding = {8, 0, 0, 0}},
            ...
            CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()},
                ...
                spacer();
                bool browse=(ui->settings_category==5&&(ui->settings_row<=3||ui->settings_row==9||ui->settings_row==10))
    ...
    CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()}, .childGap = 8}}) {
        ...
        for (int i = 0; i < 5; ++i)
            button(ui, names[i], HIT_SETTINGS_BUTTON, i, 0,
                   ui->settings_focus == 1 ? ui->settings_button == i : i == 1, true);
    }
...
static void panel(FrontendDesktopUi *ui, float height) {
    ...
    CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_GROW()},
        ...
        for (int i = start; i < (int)model.count && i < start + ui->visible_rows; ++i) {
            ...
            CLAY(id, {.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(34)},
                      ...
                      .cornerRadius = CLAY_CORNER_RADIUS(4)}) {
                CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()}}}) {
                    label(ui, c->label, 14, c->enabled ? ink : muted);
                }
                if (choice)
                    button(ui, "−", HIT_PANEL, i, -1, false, c->enabled);
                CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_PERCENT(.40f),
    ...
    label(ui, model.status ? model.status : "Arrows move or adjust • Enter selects • Escape closes", 13,
          muted);
}
        ...
        for (int row = 0; row < 4; ++row)
            CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()}, .childGap = 4}}) {
                for (int col = 0; col < 16; ++col) {
                    ...
                    CLAY(id,
                         {.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(24)},
        ...
        for (int i = 0; i < 3; ++i)
            CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()},
                ...
                CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()},
                    ...
                    CLAY(id,
                         {.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(22)},
    ...
    CLAY(CLAY_ID("desktop"),
        ...
        CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(48)},
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                button(ui, i == 1 && info.checked ? "Resume" : names[i], HIT_COMMAND, (int)commands[i], 0,
                       i == 0, enabled);
            }
            spacer();
        ...
        CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(28)},
            ...
            snprintf(status, sizeof(status), "%.100s  •  %s  •  %s%s%s%s",
                     title && *title ? title : "No game loaded", region ? region : "Ready",
                     run_state ? run_state : "Idle",
                     ui->capture && ui->capture->session.info.recording ? "  •  Recording" : "",
                     ui->execution && nes_movie_mode(ui->execution->movie) != NES_MOVIE_IDLE
                         ? "  •  Movie active" : "",
                     fds_active()?fds_loading_fast_forward()?"  •  Disk loading":fds_disk_dirty()?"  •  Disk modified":"  •  Disk":"");
            CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()}}}) {
        ...
        if (ui->open_menu >= 0) {
            ...
            int count = desktop_menu_items(ui, items), visible = (int)((h - 80) / 34);
            ...
            CLAY_AUTO_ID({.floating = {.attachTo = CLAY_ATTACH_TO_ROOT, .offset = {x, 32}, .zIndex = 1},
                ...
                for (int i = ui->menu_scroll; i < count && i < ui->menu_scroll + visible; ++i) {
                    ...
                    CLAY(id,
                        ...
                        CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW()}},
                                      .clip = {.horizontal = true, .vertical = true}}) {
                            label(ui, items[i].label, 14, items[i].enabled ? ink : muted);
                        }
                        label(ui, items[i].shortcut, 12, muted);
                    }
                }
            }
        }
        if (ui->settings_open || ui->panel_open || ui->info_open || palette_tool_is_visible()) {
            ...
            if (mh > 760)
                mh = 760;
            CLAY_AUTO_ID(
                {.floating = {.attachTo = CLAY_ATTACH_TO_ROOT, .zIndex = 2},
                 .layout = {.sizing = {.width = CLAY_SIZING_FIXED(w), .height = CLAY_SIZING_FIXED(h)},
                            .childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER}},
                 .backgroundColor = {4, 7, 13, 175}}) {
                CLAY_AUTO_ID(
                    {.layout = {.sizing = {.width = CLAY_SIZING_FIXED(mw), .height = CLAY_SIZING_FIXED(mh)},
                                .layoutDirection = CLAY_TOP_TO_BOTTOM,
        ...
        if (ui->edit_text_active || ui->capture_binding) {
            desktop_clay_block(ui->clay);
            CLAY_AUTO_ID(
                {.floating = {.attachTo = CLAY_ATTACH_TO_ROOT, .zIndex = 3},
                 .layout = {.sizing = {.width = CLAY_SIZING_FIXED(w), .height = CLAY_SIZING_FIXED(h)},
                            .childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER},
