...
struct DebugFrontend {
    ...
    WatchFrontend *watches;
    HexFrontend *hex;
    AssemblerFrontend *assembler;
    DebugToolsFrontend *tools;
    char status[256];
    char cpu_text[160];
    char stop_text[128];
...
static bool fail(DebugFrontend *frontend, char *error, size_t error_size, const char *message) {
    if (frontend) {
...
static bool parse_hex16(const char *text, uint16_t *value) {
    ...
    if (debug_symbol_resolve(text, value, NULL)) return true;
    if (text[0] == '$') {
        ++text;
    ...
    if (errno || end == text || *end || parsed > 0xFFFFu) {
        return false;
    }
...
static void sync_execution(DebugFrontend *frontend) {
    if (frontend && frontend->execution) {
...
static bool command_trace(void *context, char *error, size_t error_size) {
    ...
    snprintf(frontend->status, sizeof(frontend->status), "Trace %s", debugger_trace_enabled() ? "enabled" : "disabled");
    if (error && error_size) {
        error[0] = '\0';
    }
    return true;
...
static void refresh_breakpoints(DebugFrontend *frontend, size_t *count) {
    *count = debugger_breakpoint_count();
    if (*count > DEBUG_BREAK_MAX) {
        *count = DEBUG_BREAK_MAX;
    }
    for (size_t i = 0; i < *count; ++i) {
        DebugBreakpoint bp;
        if (!debugger_breakpoint_at(i, &bp)) {
            continue;
        }
        snprintf(frontend->breakpoint_labels[i], sizeof(frontend->breakpoint_labels[i]), "#%u %04X-%04X %c%c%c %s",
                 bp.id, bp.first, bp.last, (bp.type & DEBUG_BREAK_EXECUTE) ? 'X' : '-',
                 (bp.type & DEBUG_BREAK_READ) ? 'R' : '-', (bp.type & DEBUG_BREAK_WRITE) ? 'W' : '-',
                 bp.enabled ? "on" : "off");
...
static bool debugger_snapshot(void *context, FrontendPanelModel *model, char *error, size_t error_size) {
    DebugFrontend *frontend = context;
    DebugCpuSnapshot cpu_state;
    debugger_get_cpu(&cpu_state);
    DebugStopInfo stop = debugger_last_stop();
    snprintf(frontend->cpu_text, sizeof(frontend->cpu_text), "PC=%04X A=%02X X=%02X Y=%02X SP=%02X P=%02X cycles=%llu",
             cpu_state.cpu.pc, cpu_state.cpu.a, cpu_state.cpu.x, cpu_state.cpu.y, cpu_state.cpu.sp,
             cpu_state.cpu.status, (unsigned long long)cpu_state.cpu_cycles);
    snprintf(frontend->stop_text, sizeof(frontend->stop_text), "%s | reason=%u addr=%04X cycle=%llu",
             debugger_is_paused() ? "Paused" : "Running", (unsigned)stop.reason, stop.address,
             (unsigned long long)stop.cpu_cycle);
    ...
    FrontendPanelControl controls[] = {
        ...
        {DEBUG_CONTROL_REGISTER, FRONTEND_PANEL_TEXT, "Set register (A=FF, PC=8000)", frontend->register_edit, NULL, 0,
         0, true, false},
        {DEBUG_CONTROL_STOP, FRONTEND_PANEL_TEXT, "Debugger state", frontend->stop_text, NULL, 0, 0, true, true},
        {DEBUG_CONTROL_TOGGLE, FRONTEND_PANEL_ACTION, debugger_is_paused() ? "Resume" : "Pause", "", NULL, 0, 0, true,
         false},
        ...
        {DEBUG_CONTROL_BREAK_ADDRESS, FRONTEND_PANEL_TEXT, "Breakpoint address/range", frontend->breakpoint_address,
         NULL, 0, 0, true, false},
        {DEBUG_CONTROL_BREAK_TYPE, FRONTEND_PANEL_CHOICE, "Breakpoint type", NULL, break_types, 4,
         frontend->breakpoint_type, true, false},
        {DEBUG_CONTROL_BREAK_LIST, FRONTEND_PANEL_CHOICE, "Breakpoints", NULL, frontend->breakpoint_items, break_count,
         frontend->breakpoint_selected, break_count != 0, false},
        {DEBUG_CONTROL_BREAK_ADD, FRONTEND_PANEL_ACTION, "Add breakpoint", "", NULL, 0, 0, true, false},
        {DEBUG_CONTROL_BREAK_TOGGLE, FRONTEND_PANEL_ACTION, "Toggle selected breakpoint", "", NULL, 0, 0,
         break_count != 0, false},
        {DEBUG_CONTROL_BREAK_REMOVE, FRONTEND_PANEL_ACTION, "Remove selected breakpoint", "", NULL, 0, 0,
         break_count != 0, false},
        {DEBUG_CONTROL_TRACE, FRONTEND_PANEL_CHECKBOX, "Instruction trace", NULL, NULL, 0, debugger_trace_enabled(),
         true, false},
...
static uint8_t breakpoint_mask(int selected) {
    static const uint8_t types[] = {DEBUG_BREAK_EXECUTE, DEBUG_BREAK_READ, DEBUG_BREAK_WRITE,
...
static bool set_register(DebugFrontend *frontend, const char *value, char *error, size_t error_size) {
    ...
    if (!equals) {
        return fail(frontend, error, error_size, "Use REG=HEX, for example PC=8000");
    }
    *equals++ = '\0';
    uint32_t parsed = 0;
    char *end = NULL;
...
static bool debugger_action(void *context, unsigned id, const char *value, int selected, char *error,
                            size_t error_size) {
    DebugFrontend *frontend = context;
    if (id == DEBUG_CONTROL_REGISTER) {
        return set_register(frontend, value, error, error_size);
    }
    ...
    if (id == DEBUG_CONTROL_TOGGLE) {
        return command_toggle(frontend, error, error_size);
    }
    if (id == DEBUG_CONTROL_STEP_INTO) {
        return command_step_into(frontend, error, error_size);
    }
    if (id == DEBUG_CONTROL_STEP_OVER) {
        return command_step_over(frontend, error, error_size);
    }
    if (id == DEBUG_CONTROL_STEP_OUT) {
        return command_step_out(frontend, error, error_size);
    }
    if (id == DEBUG_CONTROL_TRACE) {
        return command_trace(frontend, error, error_size);
    }
    ...
    if (id == DEBUG_CONTROL_BREAK_ADD) {
        ...
        uint64_t key;
        uint32_t added = debug_symbol_resolve(frontend->breakpoint_address, NULL, &key)
            ? debugger_add_mapped_breakpoint((DebugBreakpointType)breakpoint_mask(frontend->breakpoint_type), first, last, key)
            : debugger_add_breakpoint((DebugBreakpointType)breakpoint_mask(frontend->breakpoint_type), first, last);
        if (!added) {
            return fail(frontend, error, error_size, "Breakpoint could not be added");
        }
    ...
    if ((id == DEBUG_CONTROL_BREAK_TOGGLE || id == DEBUG_CONTROL_BREAK_REMOVE) && frontend->breakpoint_selected >= 0 &&
        ...
        if (id == DEBUG_CONTROL_BREAK_REMOVE) {
            ...
            if (!debugger_update_breakpoint(bp.id, (DebugBreakpointType)bp.type, bp.first, bp.last, !bp.enabled)) {
                return fail(frontend, error, error_size, "Breakpoint could not be updated");
            }
            snprintf(frontend->status, sizeof(frontend->status), "Breakpoint %s", bp.enabled ? "disabled" : "enabled");
        }
        return true;
    }
...
static void format_bytes(char *out, size_t out_size, const uint8_t *bytes, size_t count, const char *prefix) {
...
static void refresh_view(DebugFrontend *frontend) {
    uint16_t address = 0;
    if (!parse_hex16(frontend->view_address, &address)) {
        address = 0;
    }
    uint8_t bytes[32] = {0};
    const char *prefix = "";
    if (frontend->view_mode == 0) {
        for (unsigned i = 0; i < 16; ++i) {
            bytes[i] = debugger_peek_cpu((uint16_t)(address + i));
        }
        prefix = "CPU: ";
    } else if (frontend->view_mode == 1) {
        for (unsigned i = 0; i < 16; ++i) {
            bytes[i] = debugger_peek_ppu((uint16_t)(address + i));
        }
        prefix = "PPU: ";
    } else if (frontend->view_mode == 2) {
        uint8_t table[0x400];
        debugger_copy_nametable((address >> 10) & 3u, table);
        memcpy(bytes, table, 16);
        prefix = "Nametable: ";
    } else if (frontend->view_mode == 3) {
        uint8_t pattern[0x1000];
        debugger_copy_pattern_table((address >> 12) & 1u, pattern);
        memcpy(bytes, pattern, 16);
        prefix = "Pattern: ";
    } else if (frontend->view_mode == 4) {
        debugger_copy_palette(bytes);
        prefix = "Palette: ";
    } else {
        uint8_t oam[256];
    ...
    if (debugger_disassemble(address, &disassembly)) {
        ...
        snprintf(frontend->view_text + used, sizeof(frontend->view_text) - used, " | %s", disassembly.text);
    }
}
...
static void refresh_watches(DebugFrontend *frontend, size_t *count) {
    ...
    *count = debugger_watch_count();
    if (*count > DEBUG_WATCH_MAX) {
        *count = DEBUG_WATCH_MAX;
    }
    frontend->watch_selected = -1;
    for (size_t i = 0; i < *count; ++i) {
        DebugWatchSample watch;
        char value[40];
        if (!debug_watch_at(i, &watch)) {
            continue;
        }
        if (watch.valid) {
            (void)debug_watch_format(watch.current, watch.spec.width, watch.spec.format, value, sizeof(value));
        } else {
            snprintf(value, sizeof(value), "%.39s", watch.error);
        }
        DebugSymbol symbol;
        const char *label = watch.spec.label;
        if (!*label && debug_symbol_find(debug_analysis_key(watch.address), &symbol)) label = symbol.name;
        snprintf(frontend->watch_labels[i], sizeof(frontend->watch_labels[i]), "#%u %04X/%u %.20s = %.39s", watch.id,
                 watch.address, watch.spec.width, label, value);
        frontend->watch_items[i] = frontend->watch_labels[i];
        if (watch.id == frontend->watch_selected_id) {
            frontend->watch_selected = (int)i;
        }
    }
    if (frontend->watch_selected < 0) {
        frontend->watch_selected_id = 0;
    }
}

static bool viewer_snapshot(void *context, FrontendPanelModel *model, char *error, size_t error_size) {
    DebugFrontend *frontend = context;
    ...
    if (trace_count) {
        ...
        if (debugger_trace_at(trace_count - 1, &entry)) {
            snprintf(frontend->trace_text, sizeof(frontend->trace_text),
                     "%zu entries | %llu PC=%04X OP=%02X A=%02X X=%02X Y=%02X", trace_count,
                     (unsigned long long)entry.cpu_cycle, entry.pc, entry.opcode, entry.a, entry.x, entry.y);
        }
    } else {
        snprintf(frontend->trace_text, sizeof(frontend->trace_text), "Trace is empty");
    }
    FrontendPanelControl controls[] = {
        ...
        {VIEW_CONTROL_SNAPSHOT, FRONTEND_PANEL_TEXT, "Snapshot + disassembly", frontend->view_text, NULL, 0, 0, true,
         true},
        {VIEW_CONTROL_WATCH_ADDRESS, FRONTEND_PANEL_TEXT, "Watch address", frontend->watch_address, NULL, 0, 0, true,
         false},
        {VIEW_CONTROL_WATCH_SIZE, FRONTEND_PANEL_CHOICE, "Watch size", NULL, sizes, 3, frontend->watch_size, true,
         false},
        ...
        {VIEW_CONTROL_WATCH_LIST, FRONTEND_PANEL_CHOICE, "Watches", NULL, frontend->watch_items, watch_count,
         frontend->watch_selected, watch_count != 0, false},
        {VIEW_CONTROL_WATCH_TOGGLE, FRONTEND_PANEL_ACTION, "Toggle selected watch", "", NULL, 0, 0,
         frontend->watch_selected >= 0, false},
        {VIEW_CONTROL_WATCH_REMOVE, FRONTEND_PANEL_ACTION, "Remove selected watch", "", NULL, 0, 0,
         frontend->watch_selected >= 0, false},
...
static bool viewer_action(void *context, unsigned id, const char *value, int selected, char *error, size_t error_size) {
    DebugFrontend *frontend = context;
    if (id == VIEW_CONTROL_MODE && selected >= 0 && selected < 6) {
        frontend->view_mode = selected;
        return true;
    }
    if (id == VIEW_CONTROL_ADDRESS || id == VIEW_CONTROL_WATCH_ADDRESS || id == VIEW_CONTROL_WATCH_LABEL) {
        ...
        size_t capacity =
            id == VIEW_CONTROL_WATCH_LABEL ? sizeof(frontend->watch_label) : sizeof(frontend->view_address);
    ...
    if (id == VIEW_CONTROL_WATCH_SIZE && selected >= 0 && selected < 3) {
        frontend->watch_size = selected;
        return true;
    }
    if (id == VIEW_CONTROL_WATCH_LIST && selected >= 0) {
        DebugWatchSample row;
        if (!debug_watch_at((size_t)selected, &row)) {
            return fail(frontend, error, error_size, "Watch is unavailable");
        }
        frontend->watch_selected = selected;
        frontend->watch_selected_id = row.id;
        return true;
    }
    if (id == VIEW_CONTROL_WATCH_ADD) {
        ...
        if (!added) {
            return fail(frontend, error, error_size, "Watch could not be added");
        }
        frontend->watch_selected = (int)debugger_watch_count() - 1;
        frontend->watch_selected_id = added;
        (void)watch_frontend_select(added);
    ...
    if (id == VIEW_CONTROL_WATCH_TOGGLE || id == VIEW_CONTROL_WATCH_REMOVE) {
        for (size_t i = 0; i < debugger_watch_count(); ++i) {
            DebugWatchSample watch;
            if (!debug_watch_at(i, &watch) || watch.id != frontend->watch_selected_id) {
                continue;
            }
            watch.spec.enabled = !watch.spec.enabled;
            bool ok = id == VIEW_CONTROL_WATCH_REMOVE ? debugger_remove_watch(watch.id)
                                                      : debug_watch_update(watch.id, &watch.spec, error, error_size);
            if (!ok) {
                return fail(frontend, error, error_size, "Watch could not be updated");
            }
            if (id == VIEW_CONTROL_WATCH_REMOVE) {
                frontend->watch_selected_id = 0;
                frontend->watch_selected = -1;
            }
        ...
        frontend->watch_selected_id = 0;
        frontend->watch_selected = -1;
        return fail(frontend, error, error_size, "The selected watch no longer exists");
    }
...
static bool lua_snapshot(void *context, FrontendPanelModel *model, char *error, size_t error_size) {
    DebugFrontend *frontend = context;
    snprintf(frontend->lua_budget, sizeof(frontend->lua_budget), "%u", debugger_lua_instruction_budget());
    snprintf(frontend->lua_state, sizeof(frontend->lua_state), "%s%s", debugger_lua_loaded() ? "Loaded" : "No script",
    ...
    snprintf(frontend->lua_error, sizeof(frontend->lua_error), "%.180s", debugger_lua_error());
    snprintf(frontend->lua_log, sizeof(frontend->lua_log), "%.244s", debugger_lua_log());
    FrontendPanelControl controls[] = {
        {LUA_CONTROL_PATH, FRONTEND_PANEL_FILE_OPEN, "Lua script path", frontend->lua_path, NULL, 0,
         FRONTEND_OPEN_SCRIPT, true, false},
...
static bool lua_action(void *context, unsigned id, const char *value, int selected, char *error, size_t error_size) {
    (void)selected;
    DebugFrontend *frontend = context;
    if (id == LUA_CONTROL_PATH) {
        ...
        strcpy(frontend->lua_path, value);
        return true;
    }
    if (id == LUA_CONTROL_LOAD) {
        return load_lua(frontend, error, error_size);
    }
    if (id == LUA_CONTROL_UNLOAD) {
        return command_lua_unload(frontend, error, error_size);
    }
...
DebugFrontend *debug_frontend_create(FrontendExecutionRuntime *execution) {
    ...
    frontend->watches = watch_frontend_create(execution);
    if (!frontend->watches) {
        free(frontend);
        return NULL;
    }
    frontend->hex = hex_frontend_create(execution);
    if (!frontend->hex) {
        watch_frontend_destroy(frontend->watches);
        free(frontend);
        return NULL;
    }
    frontend->assembler = assembler_frontend_create(execution);
    if (!frontend->assembler) {
        hex_frontend_destroy(frontend->hex);
        watch_frontend_destroy(frontend->watches);
        free(frontend);
        return NULL;
    }
    frontend->tools = debug_tools_frontend_create(execution);
    if (!frontend->tools) {
        assembler_frontend_destroy(frontend->assembler);
        hex_frontend_destroy(frontend->hex);
        watch_frontend_destroy(frontend->watches);
        free(frontend);
        return NULL;
...
bool debug_frontend_register_ui(DebugFrontend *frontend) {
    ...
    const FrontendCommandSpec commands[] = {{DEBUGGER_FRONTEND_COMMAND, "Debugger Pause/Resume", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_toggle, frontend},
                                            {DEBUGGER_VIEWERS_FRONTEND_COMMAND, "Toggle Instruction Trace", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION | FRONTEND_COMMAND_CHECKABLE, command_trace,
                                             frontend},
                                            {DEBUGGER_LUA_FRONTEND_COMMAND, "Load Lua Script", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_lua_load, frontend},
                                            {DEBUGGER_STEP_INTO_COMMAND, "Debugger Step Into", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_step_into, frontend},
                                            {DEBUGGER_STEP_OVER_COMMAND, "Debugger Step Over", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_step_over, frontend},
                                            {DEBUGGER_STEP_OUT_COMMAND, "Debugger Step Out", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_step_out, frontend},
                                            {DEBUGGER_RESUME_COMMAND, "Debugger Resume", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_resume, frontend},
                                            {DEBUGGER_PAUSE_COMMAND, "Debugger Pause", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_pause, frontend},
                                            {DEBUGGER_LUA_LOAD_COMMAND, "Load Configured Lua Script", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_lua_load, frontend},
                                            {DEBUGGER_LUA_UNLOAD_COMMAND, "Unload Lua Script", "Tools", "",
                                             FRONTEND_COMMAND_NEEDS_SESSION, command_lua_unload, frontend}};
    size_t registered_commands = 0;
    for (; registered_commands < sizeof(commands) / sizeof(commands[0]); ++registered_commands) {
        if (!frontend_command_register(&commands[registered_commands])) {
            break;
        }
    }
    if (registered_commands != sizeof(commands) / sizeof(commands[0])) {
        while (registered_commands) {
            (void)frontend_command_unregister(commands[--registered_commands].id);
        }
        return false;
    }
    const FrontendPanelSpec panels[] = {{DEBUGGER_FRONTEND_PANEL, "Debugger", "Tools", FRONTEND_PANEL_NEEDS_SESSION,
                                         debugger_snapshot, debugger_action, frontend},
                                        {DEBUGGER_VIEWERS_FRONTEND_PANEL, "Memory and PPU Viewers", "Tools",
                                         FRONTEND_PANEL_NEEDS_SESSION, viewer_snapshot, viewer_action, frontend},
                                        {DEBUGGER_LUA_FRONTEND_PANEL, "Lua Scripting", "Tools",
                                         FRONTEND_PANEL_NEEDS_SESSION, lua_snapshot, lua_action, frontend}};
    size_t registered_panels = 0;
    for (; registered_panels < sizeof(panels) / sizeof(panels[0]); ++registered_panels) {
        if (!frontend_panel_register(&panels[registered_panels])) {
            break;
        }
    }
    if (registered_panels != sizeof(panels) / sizeof(panels[0])) {
        while (registered_panels) {
            (void)frontend_panel_unregister(panels[--registered_panels].id);
        }
    ...
    if (!desktop_ppu_register()) {
        for (size_t i = 0; i < sizeof(panels) / sizeof(panels[0]); ++i) {
            frontend_panel_unregister(panels[i].id);
        }
        for (size_t i = 0; i < sizeof(commands) / sizeof(commands[0]); ++i) {
            frontend_command_unregister(commands[i].id);
        }
        return false;
    }
    if (!hex_frontend_register(frontend->hex)) {
    ...
    if (!assembler_frontend_register(frontend->assembler)) {
        hex_frontend_unregister(frontend->hex);
        desktop_ppu_unregister();
        for (size_t i = 0; i < sizeof(panels) / sizeof(panels[0]); ++i) {
            frontend_panel_unregister(panels[i].id);
        }
    ...
    if (!debug_tools_frontend_register(frontend->tools)) {
        assembler_frontend_unregister(frontend->assembler);
        hex_frontend_unregister(frontend->hex);
        desktop_ppu_unregister();
    ...
    if (!watch_frontend_register(frontend->watches)) {
        debug_tools_frontend_unregister(frontend->tools);
        assembler_frontend_unregister(frontend->assembler);
        hex_frontend_unregister(frontend->hex);
        desktop_ppu_unregister();
        for (size_t i = 0; i < sizeof(panels) / sizeof(panels[0]); ++i) {
...
void debug_frontend_image_changed(DebugFrontend *frontend) {
    ...
    frontend->watch_selected = -1;
    frontend->watch_selected_id = 0;
    watch_frontend_image_changed(frontend->watches);
    hex_frontend_image_changed(frontend->hex);
    assembler_frontend_image_changed(frontend->assembler);
    debug_tools_frontend_image_changed(frontend->tools);
    frontend->status[0] = '\0';
}
...
void debug_frontend_destroy(DebugFrontend *frontend) {
    ...
    if (frontend->registered) {
        desktop_ppu_unregister();
        const unsigned commands[] = {DEBUGGER_FRONTEND_COMMAND,     DEBUGGER_VIEWERS_FRONTEND_COMMAND,
    ...
    debugger_lua_unload();
    debug_tools_frontend_destroy(frontend->tools);
    assembler_frontend_destroy(frontend->assembler);
    hex_frontend_destroy(frontend->hex);
    watch_frontend_destroy(frontend->watches);
    free(frontend);
