...
static void activate_menu_row(FrontendDesktopUi *ui, int row) {
    ...
    char error[256] = {0};
    if (item->kind == 0)
        (void)desktop_invoke_command(ui, item->id);
    else if (item->kind == 1) {
        ui->panel_id = item->id;
...
static void activate_panel(FrontendDesktopUi *ui, int row, int direction) {
    ...
    char error[256] = {0};
    if (!frontend_panel_snapshot(ui->panel_id, &model, error, sizeof(error)) || row < 0 ||
        (size_t)row >= model.count)
        return;
    ui->panel_row = row;
    FrontendPanelControl *control = &controls[row];
    if (!direction || !control->enabled || control->read_only)
        return;
    ...
    if ((control->type == FRONTEND_PANEL_CHOICE || control->type == FRONTEND_PANEL_LIST) &&
        control->item_count)
...
static void browse_setting(FrontendDesktopUi *ui) {
    ...
    if (ui->settings_category == 5 && (row <= 3 || row == 9))
        chosen = frontend_open_file_dialog(row == 9 ? FRONTEND_OPEN_TAPE : FRONTEND_OPEN_FIRMWARE, path,
        ...
        chosen = frontend_save_file_dialog(row == 1 ? FRONTEND_SAVE_STATE : (FrontendSaveFileType)(row - 2),
...
static bool handle_click(FrontendDesktopUi *ui, int x, int y) {
    ...
    case HIT_CHANNEL: {
        ...
        int value = hit.direction ? rgb[hit.index] + hit.direction
                                  : (int)((x - hit.bounds.x) * 255 / hit.bounds.width);
...
static bool handle_event(FrontendDesktopUi *ui, const SDL_Event *event) {
    ...
    if (event->type == SDL_DROPFILE && frontend_desktop_input_captured(ui)) {
        if (event->drop.file) {
            if (palette_tool_is_visible())
                desktop_copy_status(ui, ppu_palette_load_pal_file(event->drop.file) == 0
                                            ? "Palette loaded"
    ...
    if (event->type == SDL_KEYDOWN && !event->key.repeat && !ui->edit_text_active && !ui->capture_binding &&
        ...
        if (palette_tool_is_visible() && sc == SDL_SCANCODE_F6) {
            ppu_palette_reset_default();
            return true;
        }
        if (palette_tool_is_visible() && sc == SDL_SCANCODE_V && (event->key.keysym.mod & KMOD_CTRL)) {
    ...
    if (event->type == SDL_WINDOWEVENT && ui->window &&
        event->window.windowID == SDL_GetWindowID(ui->window)) {
        if (event->window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
            frontend_desktop_update_window_settings(ui);
            desktop_sync_scale(ui);
        }
        if (event->window.event == SDL_WINDOWEVENT_FOCUS_LOST && ui->settings->pause_on_focus_loss &&
            ui->execution && !frontend_execution_paused(ui->execution)) {
            ui->paused_for_focus = desktop_invoke_command(ui, FRONTEND_COMMAND_PAUSE);
        }
        if (event->window.event == SDL_WINDOWEVENT_FOCUS_GAINED && ui->paused_for_focus && ui->execution &&
            frontend_execution_paused(ui->execution)) {
            (void)desktop_invoke_command(ui, FRONTEND_COMMAND_PAUSE);
            ui->paused_for_focus = false;
        }
    }
    if (event->type == SDL_MOUSEWHEEL && (ui->edit_text_active || ui->capture_binding)) return true;
    ...
    if (event->type == SDL_KEYDOWN && !event->key.repeat) {
        ...
        if (sc == SDL_SCANCODE_ESCAPE) {
            if (ui->settings_open)
                ...
                ui->info_open = false;
            else if (palette_tool_is_visible())
                palette_tool_hide_overlay();
            else
                ui->open_menu = -1;
            return true;
...
bool frontend_desktop_handle_event(FrontendDesktopUi *ui, const SDL_Event *event) {
    ...
    if (ui && ui->execution && ui->settings && ui->settings->pause_on_ui) {
        if (!before && after && !frontend_execution_paused(ui->execution))
