...
Project-owned source files should stay at or below 1,500 lines. Split a file at subsystem or test-group boundaries before it grows past that limit. Private implementation headers keep helpers and state in one translation unit without turning them into public interfaces. Imported components keep their upstream layout and formatting markers. Add a filename, `Author: @frankischilling`, and GPL-3.0-or-later header to new project-owned C/C++ files. Run `python scripts/check-source-size.py` and `python scripts/check-source-headers.py` locally; CI runs both checks.
